...
export interface ReplicadLike extends GenericRecord {
    ...
    getFont?: () => unknown;
    loadFont?: (fontData: string | ArrayBuffer, fontName?: string, forceUpdate?: boolean) => Promise<unknown>;
...
export interface CodeEvaluator {
    runInContext<T = unknown>(code: string, context?: Record<string, unknown>): T | Promise<T>;
    evaluateModule<TModule extends GenericRecord = GenericRecord>(code: string, options?: EvaluateModuleOptions): Promise<TModule>;
}
...
export interface EvaluatorService {
    ...
    exportShape: (fileType?: string, shapeId?: string, meshConfig?: GenericRecord) => Promise<any[]>;
    loadFont: (fontData: string | ArrayBuffer, fontName?: string, forceUpdate?: boolean) => Promise<void>;
    faceInfo: (subshapeIndex: number, faceIndex: number, shapeId?: string) => GenericRecord | null;
    edgeInfo: (subshapeIndex: number, edgeIndex: number, shapeId?: string) => GenericRecord | null;
}
