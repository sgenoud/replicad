...
function main(
  ...
  const buildSocket = ({
    ...
    const baseSocket = roundedRectangleBlueprint(
      SIZE - CLEARANCE,
      SIZE - CLEARANCE,
      CORNER_RADIUS
    ).sketchOnPlane();
    ...
    if (withScrew || withMagnet) {
      const magnetCutout = withMagnet
        ? sketchCircle(magnetRadius).extrude(magnetHeight)
        : null;
      const screwCutout = withScrew
        ? sketchCircle(screwRadius).extrude(SOCKET_HEIGHT)
        : null;
  ...
  const buildTopShape = ({
    ...
    const topShape = (basePlane, startPosition) => {
      const sketcher = new BlueprintSketcher()
        .movePointerTo([-SOCKET_TAPER_WIDTH, 0])
        .line(SOCKET_SMALL_TAPER, SOCKET_SMALL_TAPER)
        .vLine(SOCKET_VERTICAL_PART)
      ...
      const shiftedShape = intersectBlueprints(
        basicShape.translate(AXIS_CLEARANCE, -AXIS_CLEARANCE),
        roundedRectangleBlueprint(10, 10).translate(-5, includeLip ? 0 : 5)
      );

      // We need to shave off the clearance
      let topProfile = intersectBlueprints(
        shiftedShape.translate(CLEARANCE / 2, 0),
        roundedRectangleBlueprint(10, 10).translate(-5, 0)
      );

      if (includeLip) {
        // We remove the wall if we add a lip
        topProfile = cutBlueprints(
          topProfile,
          roundedRectangleBlueprint(1.2, 10).translate(-0.6, -5)
        );
      }
    ...
    const boxSketch = roundedRectangleBlueprint(
